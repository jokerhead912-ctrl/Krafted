...
run() {   # run(label)
  local out
  out=$(KRAFTED_HTML=$TMP/mut.html KRAFTED_SW=$TMP/sw.js $NODE Krafted/tests/test_v7046.js 2>&1)
...
mutate "the Present button stops naming its key" \
  'title="Present the views — you advance it, Space or → for the next shot (Shift+P)"' \
  'title="Present the views — you advance it, Space or → for the next shot"'
...
mutate "KRAFTED_VERSION not bumped" \
  "var KRAFTED_VERSION = '7.10.1';" \
  "var KRAFTED_VERSION = '7.10.0';"
