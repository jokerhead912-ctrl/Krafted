...
(function () {
  ...
  codeCount('newGroup = { id: gid, color, memberIds:', 0, 'the v7.16.0 newGroup literal is gone too');
...
section(function () {
  ...
  (function () {
    ...
    eq(g.labelEl.style.display, 'block', 'v7.16.0: beginning a rename shows the chip (a nameless one starts hidden)');
    ok(borderPlacements >= 1, 'v7.16.0: beginning a rename re-places the chip through updateGroupBorder');
...
section(function () {
  ...
  has("const editing = group.labelEl.classList.contains('editing');\n    if (!group.name && !editing) {",
    'v7.16.0: the guard is nameless AND not-editing, so a mid-rename chip stays placed');
