...
ok(src.indexOf("var KRAFTED_VERSION = '7.10.1';") >= 0, 'KRAFTED_VERSION bumped');
ok(src.indexOf('<title>Krafted v7.10.1</title>') >= 0, 'title bumped');
const sw = fs.readFileSync(path.resolve(__dirname, '../../Krafted/docs/sw.js'), 'utf8');
ok(sw.indexOf("const CACHE_NAME = 'krafted-v7.10.1-'") >= 0, 'sw CACHE_NAME bumped');
ok(sw.indexOf("const APP_VERSION = '7.10.1';") >= 0, 'sw APP_VERSION bumped');
