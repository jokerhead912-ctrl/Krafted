...
// ═══════════════════════════════════════════════════════════════════════
{
  ok(src.indexOf("var KRAFTED_VERSION = '7.0.50';") >= 0, 'KRAFTED_VERSION bumped');
  ok(src.indexOf('<title>Krafted v7.0.50</title>') >= 0, 'title bumped');
  const swPath = process.env.KRAFTED_SW
    ? path.resolve(process.env.KRAFTED_SW)
    : path.resolve(__dirname, '../docs/sw.js');
  const sw = fs.readFileSync(swPath, 'utf8');
  ok(sw.indexOf("const CACHE_NAME = 'krafted-v7.0.50-'") >= 0, 'sw CACHE_NAME bumped');
  ok(sw.indexOf("const APP_VERSION = '7.0.50';") >= 0, 'sw APP_VERSION bumped');
}
