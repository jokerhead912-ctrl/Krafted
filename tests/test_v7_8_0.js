...
section(function () {
  ...
  ok(HTML.indexOf('// v7.23.0:') > 0, 'v7.23.0 provenance comments are present');
});
...
section(function () {
  ...
  near(p00.x, e00.x, 'v7.23.0: source pixel (0,0) lands on the rotated item’s top-left corner (x)', 0.02);
...
asection(async function () {
  ...
  eq(newItem.rot, 0, 'v7.23.0: the extracted copy is upright (rot = 0)');
...
asection(async function () {
  ...
  eq(log.fill[0], 'destination-out', 'v7.23.0: the hole is SUBTRACTED (destination-out), not painted over');
