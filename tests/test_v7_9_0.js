...
section(function () {
  ...
  has("if (_selImages) html += exportMenuEntries(_selImages);",
    'the image entries appear only for items the export can actually see (v7.21.0)');
...
section(function () {
  ...
  const got = env.links;
  eq(got.length, 5, 'only the five media items produce a download (v7.21.0: images go to the export engine)');
