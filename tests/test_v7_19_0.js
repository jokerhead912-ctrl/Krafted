...
section('S1c: the FULL style path syncs through the same helper', () => {
  ...
  eq(styleApi._docClassCalls.length, 1, 'the full path still re-derives doc-card classes (v7.25.0 behaviour kept)');
});
...
section('S3: the canvas drag lifecycle carries the two-stage wiring', () => {
  ...
  eq(c.split('_textEditCandidate').length - 1, 0,
    'the micro-click edit candidate is GONE (v7.25.0: edit is double-click only)');
...
section('S5: v7.25.0 anchors untouched', () => {
  const c = codeOnly(src);
