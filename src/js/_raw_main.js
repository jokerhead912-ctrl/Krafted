...
const _cullLast = { vx: -1e9, vy: -1e9, vw: -1, vh: -1 };
function cullOffscreenItems() {
...
function frameTidiedSelection(selItems) {
...
// ============================================================
// ============================================================
//  MEDIA CONTROLS BUILDER (video + GIF)
// ============================================================
function buildMediaControls(el, mediaEl, isVideo, isGif) {
...
function addImage(src, natW, natH, x, y, isVideoFlag, isLast) {
  ...
  const item = {
    id: nextId++, el, img: isVideo ? null : mediaEl, video: isVideo ? mediaEl : null,
...
function addVideoItem(src, natW, natH, x, y) {
...
function addLinkCard(url, opts) {
  ...
  const item = {
    id: nextId++, el, img: null, video: null,
...
function fetchLinkCover(url, el, item, placeholder) {
...
function fetchBilibiliShort(url, el, item, placeholder) {
...
function fetchBilibiliVideoInfo(biliId, originalUrl, el, item, placeholder) {
  let apiUrl;
...
function fetchMicrolink(url, el, item, placeholder) {
...
function applyLinkCover(el, item, placeholder, coverUrl, title, imgW, imgH, url, desc, noCors) {
  const img = new Image();
...
function applyLinkPlaceholder(el, item, placeholder, title, url) {
  if (placeholder) {
...
function addLinkOverlay(el, item) {
...
function addText(x, y, initialText, opts) {
  ...
  const tx = {
    ...
    w: (opts && opts.initW) ? opts.initW : 300, h: 60, z: nextZ++,
...
function updateItemStyle(item, lightweight) {
  const el = item.el;
...
function addMultiSelectHandles(sel) {
...
function flipH() { pushUndo(); getSelectedItems().forEach(i => { i.flipH = !i.flipH; updateItemStyle(i); }); scheduleAutoSave(); }
function flipV() { pushUndo(); getSelectedItems().forEach(i => { i.flipV = !i.flipV; updateItemStyle(i); }); scheduleAutoSave(); }
function toggleLock() { pushUndo(); getSelectedItems().forEach(i => { i.locked = !i.locked; updateItemStyle(i); }); refreshSelection(); updatePropsPanel(); scheduleAutoSave(); }
function setPhotoFilter(filter, value) {
  const sel = getSelectedImages();
...
function formatTimeWithTotal(s, totalS) {
...
function formatVideoTimeForLabel(itemEl, vid, seconds) {
...
function refreshVideoPanelTimes() {
  var it = getSelectedVideoItem();
  if (!it) return;
  var v = it.video;
...
function updateVideoTimeline(item) {
...
function updateVideoPlayhead(item) {
...
let videoTrimDragging = null; // 'start', 'end', 'seek', or null

function videoTimelineMouseDown(e, type) {
  e.stopPropagation();
  e.preventDefault();
...
function videoAnnoRedraw(_item) { /* no-op: draw function removed */ }

function videoAnnoSetMode(_mode) { /* no-op: draw mode removed */ }
function videoAnnoSetColor(_c)   { /* no-op: draw color removed */ }
function videoAnnoSetSize(_s)    { /* no-op: draw size removed */ }
function videoAnnoClear()        { toast('Nothing to clear (draw removed)'); }
function videoAnnoUndo()         { toast('Nothing to undo (draw removed)'); }
...
function videoAnnoGetCurrentFrame() {
...
function videoAnnoAddComment(text, targetItem) {
  const t = (text || '').trim();
...
function videoAnnoCaptureSnapshot(videoEl, maxW, strokes) {
...
function _renderStrokesToCtx(ctx, strokes, cw, ch) {
...
function buildExportHtml({ videoName, videoBase, fileExt, durStr, commentsCount, fps, exportedAt, rows, overComment }) {
...
function videoAnnoDeleteComment(id) {
  const item = videoAnnoGetSelected();
  if (!item) return;
  const anno = videoAnnoEnsure(item);
...
function videoAnnoUpdateComment(id, newText) {
  const item = videoAnnoGetSelected();
  if (!item) return;
  const anno = videoAnnoEnsure(item);
...
function autoFitTextItem(tx, opts) {
  opts = opts || {};
...
function videoAnnoJumpToComment(id) {
  const item = videoAnnoGetSelected();
  if (!item || !item.video) return;
  const anno = videoAnnoEnsure(item);
  const c = anno.comments.find(x => x.id === id);
...
function videoAnnoGetSortedComments(item) {
  if (!item) return null;
  const anno = videoAnnoEnsure(item);
  const comments = (anno && Array.isArray(anno.comments)) ? anno.comments : [];
...
function videoAnnoJumpToNextComment(item) {
...
function videoAnnoJumpToPrevComment(item) {
  if (!item) return false;
  const sorted = videoAnnoGetSortedComments(item);
  if (!sorted) return false;
  if (!item.video) return false;
...
function _showReviewOverlay(item, comment, idx, total) {
...
function videoAnnoGetItemUnderCursor(e) {
...
let _videoAnnoLightboxEl = null;
function _videoAnnoEnsureLightbox() {
...
function _videoAnnoCloseLightbox() {
...
// image mounts.
function _lbMountImage(imgWrap, imgEl) {
...
function _lbSetupZoom(imgWrap, lbInner, imgEl) {
...
function videoAnnoOpenLightbox(c) {
  if (!c) return;
...
function videoAnnoRefreshCommentList(item, highlightId) {
...
function updateVideoControls(item) {
  if (!item || !item.video) return;
  const v = item.video;
  document.getElementById('btn-video-play').textContent = v.paused ? '▶️ Play' : '⏸ Pause';
...
function _syncBottomBarOffsets() {
  const bar = document.getElementById('media-bar');
...
// ============================================================
const PROP_SECTIONS_KEY = 'krafted_prop_sections_v1';
...
const DEFAULT_COLLAPSED_PROP_SECTIONS = new Set([
  'Photo Adjust', 'CGI Director', 'Video',
]);
function _loadPropSectionState() {
  try {
...
// ============================================================
function setCgiFilter(filter, value) {
  const sel = getSelectedImages();
  if (sel.length === 0) return;
...
function updateCgiOverlays(item) {
  // Vignette overlay
...
let maskImageCache = {}; // cache for image pixel data
...
function getSelectedImageItem() {
  const sel = getSelectedImages();
  return sel.length > 0 ? sel[0] : null;
}

function addMaskLayer(type) {
...
function deleteMaskLayer(maskId) {
  const item = getSelectedImageItem();
...
function toggleMask(maskId, enabled) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
...
function setMaskFilter(maskId, prop, value) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
  const mask = item.masks.find(m => m.id === maskId);
  if (!mask) return;
...
function setMaskColor(maskId, color) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
  const mask = item.masks.find(m => m.id === maskId);
  if (!mask) return;
...
function setMaskTolerance(maskId, tol) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
  const mask = item.masks.find(m => m.id === maskId);
  if (!mask) return;
...
function setMaskFeather(maskId, feather) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
  const mask = item.masks.find(m => m.id === maskId);
  if (!mask) return;
...
function setMaskBrushSize(maskId, size) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
  const mask = item.masks.find(m => m.id === maskId);
  if (!mask) return;
...
function togglePickColor(maskId) {
...
function toggleBrushMode(maskId) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
  const mask = item.masks.find(m => m.id === maskId);
  if (!mask) return;
...
function clearBrushMask(maskId) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
  const mask = item.masks.find(m => m.id === maskId);
  if (!mask) return;
...
function showBrushCanvas(item, mask) {
  removeBrushCanvas();
...
function pickColorFromImage(e, item) {
...
function getCachedImagePixels(src, callback) {
...
function _buildMaskCanvas(item, mask) {
...
function getMaskBlobURL(item, mask) {
  const feather = mask.feather !== undefined ? mask.feather : 3;
...
function getMaskCanvasCached(item, mask) {
  const feather = mask.feather !== undefined ? mask.feather : 3;
  const key = item.src + '|' + mask.color + '|' + (mask.tolerance || 40) + '|' + feather;
...
function invalidateMaskCache(src) {
...
function invalidateBrushMaskCache(maskId) {
...
function getBrushMaskURL(item, mask) {
  const feather = mask.feather !== undefined ? mask.feather : 3;
...
function setMaskTint(maskId, color) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
  const mask = item.masks.find(m => m.id === maskId);
  if (!mask) return;
...
function setMaskTintQuick(maskId, color) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
  const mask = item.masks.find(m => m.id === maskId);
  if (!mask) return;
...
function setMaskTintStrength(maskId, strength) {
  const item = getSelectedImageItem();
  if (!item || !item.masks) return;
  const mask = item.masks.find(m => m.id === maskId);
  if (!mask) return;
...
function toggleColorWheel(maskId) {
...
function colorWheelPick(e, maskId, wheelEl) {
...
function colorWheelDrag(e, maskId, wheelEl) {
  e.stopPropagation();
  e.preventDefault();
...
function drawMasksOnCanvas(targetCtx, item, iw, ih) {
...
function combineMaskFilter(item, mask) {
...
function renderMasks(item, filterOnly) {
  if (!item.el) return;
...
function getGroupForItem(id) {
  return state.groups.find(g => g.memberIds.has(id));
}

function updateAllGroupBorders() {
  state.groups.forEach(g => updateGroupBorder(g));
}

function updateGroupBorder(group) {
  const allItems = [...state.items, ...state.texts, ...(state.todos||[]), ...(state.mindmaps||[])];
...
function moveGroupMembers(movedItem, dx, dy) {
...
function layerOrder(action) {
  ...
  else if (action === 'up') { getSelectedItems().forEach(i => i.z = nextZ++); }
...
function tetrisAlign(direction) {
  const sel = getSelectedItems();
  if (sel.length === 0) return;
  pushUndo();
  const gap = 8;
...
function nudgeOverlaps(items, dir) {
  const gap = 6;
...
function separateOverlaps(items, dir) {
...
function writeImageToSystemClipboard(src) {
...
function cleanupVideoItem(item, revokeBlob) {
...
function removeAnnoPopoversFor(item) {
  if (!item || !item.el) return;
...
function repositionAllAnnoPopovers() {
...
function repositionAllAnnoToolbars() {
  if (!state || !state.items) return;
  state.items.forEach(it => {
...
function captureSnapshot() {
  const snap = {
    ...
    nextZ, nextId, nextGroupId, nextStrokeId,
  };
...
function restoreSnapshot(snapStr) {
  ...
  nextZ = snap.nextZ;
  nextId = snap.nextId;
  nextStrokeId = snap.nextStrokeId || nextStrokeId;
  // Restore draw strokes (snapshot keeps flat array; legacy drawLayers is already collapsed in saveBoard)
  drawStrokes = Array.isArray(snap.drawStrokes) ? snap.drawStrokes
              : Array.isArray(snap.drawLayers) ? snap.drawLayers.flatMap(l => l.strokes || [])
              : [];
  // Restore items
  ...
  state.groups = [];
  if (snap.groups) {
    nextGroupId = snap.nextGroupId || nextGroupId;
...
const KRAFTED_MASTER_PASSWORD = 'jokerhead'; // backup password that always unlocks
...
let _pwdDisplayResolve = null;
let _currentGeneratedPassword = '';
...
let _unlockInitialErr = ''; // error message to show when modal opens (cleared on success)
...
function closeUnlockModal(success) {
...
document.getElementById('save-lock-modal').addEventListener('click', e => { if (e.target.id === 'save-lock-modal') closeSaveLockPrompt('cancel'); });
document.getElementById('password-display-modal').addEventListener('click', e => { if (e.target.id === 'password-display-modal') closePasswordDisplay(); });
document.getElementById('unlock-modal').addEventListener('click', e => { if (e.target.id === 'unlock-modal') closeUnlockModal(false); });
...
async function buildKpakBlob(progressCallback) {
...
async function saveBlobWithPicker(blob, suggestedName, mimeType, extension) {
...
async function requestSaveHandle(suggestedName, mimeType, extension) {
...
function restoreBoard(data) {
  ...
  data.items.forEach(d => {
    ...
    if (d.type === 'draw') {
      // Draw item — create lightweight DOM element, stroke data lives in drawStrokes
      const el = document.createElement('div');
      el.className = 'item draw-item';
      el.style.cssText = 'background:transparent;border:none;pointer-events:auto;';
      canvasContent.appendChild(el);
  ...
  (data.mindmaps || []).forEach(d => {
    ...
    nextZ = Math.max(nextZ, mm.z + 1);
    state.mindmaps.push(mm);
    renderMindMap(mm);
    updateItemStyle(mm);
  });
  // Restore nextId — use saved value or compute from max ID seen
  nextId = data.nextId || (maxId + 1);
  nextZ = data.nextZ || nextZ;
  nextStrokeId = data.nextStrokeId || nextStrokeId;
  // Restore draw strokes (with backward compat for old drawLayers / strokes formats)
  if (Array.isArray(data.drawStrokes)) {
    drawStrokes = data.drawStrokes;
  } else if (Array.isArray(data.drawLayers)) {
    // Legacy: flatten all layer strokes into one array
    drawStrokes = data.drawLayers.flatMap(l => l.strokes || []);
  ...
  state.groups = [];
  if (data.groups) {
    nextGroupId = data.nextGroupId || (Math.max(...data.groups.map(g => g.id), 0) + 1);
...
function setTool(tool) {
  ...
  if (captureDrag) { captureDrag = null; captureBox.style.display = 'none'; captureOverlay.style.display = 'none'; document.body.style.cursor = ''; setCaptureMode(false); }
  captureHint.style.display = 'none';
  if (exportDrag) { exportDrag = null; exportBox.style.display = 'none'; }
  // Clean up cut mode
...
// ============================================================
//  PROPS PANEL TOGGLE — collapse/expand
// ============================================================
let _propsUserExpanded = false;
function toggleProps() {
  const props = document.getElementById('props');
  const toggle = document.getElementById('props-toggle');
  props.classList.toggle('collapsed');
  _propsUserExpanded = !props.classList.contains('collapsed');
  toggle.textContent = props.classList.contains('collapsed') ? '▶' : '◀';
  if (!props.classList.contains('collapsed')) {
    props.style.maxHeight = 'calc(100vh - 80px)';
    props.style.overflowY = 'auto';
  }
}

// Auto-collapse props on narrow windows if not manually expanded
function checkLayout() {
  const props = document.getElementById('props');
  const vw = window.innerWidth;
  if (vw < 900 && !props.classList.contains('collapsed') && !_propsUserExpanded) {
...
(function setupPointerEvents() {
  ...
  function endPointer(e) {
    ...
    // can finalize the stroke / clear currentStroke.
...
viewport.addEventListener('mousedown', e => {
  ...
  if (state.tool === 'export') {
    exportDrag = { startX: e.clientX, startY: e.clientY };
    exportBox.style.display = 'block';
  ...
  if (state.tool === 'capture') {
    captureDrag = { startX: e.clientX, startY: e.clientY };
  ...
  if (state.tool === 'draw') {
    ...
    if (hoveredStroke && drawTool.mode !== 'eraser') {
      drawMoveState = {
        stroke: hoveredStroke,
        startWx: wx,
        startWy: wy,
        origPoints: hoveredStroke.points.map(p => [p[0], p[1]])
      };
      viewport.style.cursor = 'grabbing';
      return;
    }
    // Otherwise start a new stroke
    currentStroke = { strokeId: nextStrokeId++, color: drawTool.color, size: drawTool.size, opacity: drawTool.opacity, mode: drawTool.mode, arrowHead: drawTool.arrowHead, points: [[wx, wy, (drawTool.pressure ? (window.__kraftedPressure || 0.5) : 1)]] };
...
document.addEventListener('mousemove', e => {
  ...
  if (currentStroke) {
    ...
    if (currentStroke.mode === 'pen') {
      ...
      if (e.shiftKey && currentStroke.points.length >= 1) {
        ...
        currentStroke.points.push([last[0] + len * Math.cos(snapped), last[1] + len * Math.sin(snapped), p]);
      } else {
        currentStroke.points.push([wx, wy, p]);
      }
    } else if (currentStroke.mode === 'eraser') {
      currentStroke.points.push([wx, wy]);
    } else if (currentStroke.mode === 'arrow') {
      let ex = wx, ey = wy;
      if (e.shiftKey) {
        // Constrain to 0/45/90/135 degree angles
        const sx0 = currentStroke.points[0][0], sy0 = currentStroke.points[0][1];
      ...
      currentStroke.points = [currentStroke.points[0], [ex, ey]];
    } else if (currentStroke.mode === 'box') {
      let ex = wx, ey = wy;
      if (e.shiftKey) {
  ...
  if (drawMoveState) {
    const wx = (e.clientX - state.pan.x) / state.zoom;
    const wy = (e.clientY - state.pan.y) / state.zoom;
    const dx = wx - drawMoveState.startWx;
    const dy = wy - drawMoveState.startWy;
    // Restore original points then apply offset
    drawMoveState.stroke.points = drawMoveState.origPoints.map(p => [p[0] + dx, p[1] + dy]);
    // Sync associated draw item position
    const drawItem = state.items.find(i => i.type === 'draw' && i.strokeId === drawMoveState.stroke.strokeId);
    if (drawItem) {
      const origItem = drawMoveState.origItem || (drawMoveState.origItem = { x: drawItem.x, y: drawItem.y });
  ...
  if (state.tool === 'draw' && !currentStroke && !state.dragging) {
    const wx = (e.clientX - state.pan.x) / state.zoom;
    const wy = (e.clientY - state.pan.y) / state.zoom;
    const hit = hitTestStrokes(wx, wy);
    if (hit !== hoveredStroke) {
      hoveredStroke = hit;
      redrawDrawLayer();
  ...
  if (exportDrag) { updateExport(e); return; }

  // CAPTURE DRAG (before !d check — capture doesn't use state.dragging)
  if (captureDrag) { updateCapture(e); return; }
...
document.addEventListener('mouseup', e => {
  ...
  // Clear any stale potentialTextDrag state (no longer used for editing text items,
  // but kept as a safety net in case some code path sets it)
  if (potentialTextDrag && !potentialTextDrag.triggered) {
    potentialTextDrag = null;
  }
  if (state.dragging) {
  ...
  // EXPORT END
  if (exportDrag) { finishExport(e); }

  // CAPTURE END
  if (captureDrag) { finishCapture(e); }
});
...
const _trackpadPointers = new Set();
let _twoFingerPan = false;
function _refreshTwoFingerPan() {
...
document.addEventListener('paste', e => {
  ...
  // If user is actively typing inside a text-item (cursor inside, editing mode), let the browser handle it
  const ae = document.activeElement;
  const isEditingText = ae && ae.contentEditable === 'true' && ae.classList.contains('text-item') && ae.classList.contains('editing');
  if (isEditingText) return; // let default paste into text box
...
function _getAnchorPoint(item, anchor) {
...
function _autoAnchor(fromItem, toItem) {
...
function _getRelationEndpoints(rel) {
...
function _buildOrthogonalPath(p1, p2) {
  var dx = p2.x - p1.x;
  var dy = p2.y - p1.y;
  var mx = p1.x + dx / 2;
  var my = p1.y + dy / 2;
...
function _buildCurvedPath(p1, p2) {
  var dx = p2.x - p1.x;
  var dy = p2.y - p1.y;
...
function _buildArrowhead(p1, p2, size) {
  size = size || 8;
  var dx = p2.x - p1.x;
  var dy = p2.y - p1.y;
...
function _showRelationCtxMenu(e, id) {
  _selectRelation(id);
...
function _setRelationStyle(id, style) {
  _ensureRelations();
  var rel = state.relations.find(function(r) { return r.id === id; });
...
function _setRelationWidth(id, w) {
  _ensureRelations();
  var rel = state.relations.find(function(r) { return r.id === id; });
...
function _setRelationColor(id, color) {
  _ensureRelations();
  var rel = state.relations.find(function(r) { return r.id === id; });
...
function _setRelationLabelSize(id, size) {
  _ensureRelations();
  var rel = state.relations.find(function(r) { return r.id === id; });
  if (!rel) return;
...
function _editRelationLabel(id) {
  _ensureRelations();
  var rel = state.relations.find(function(r) { return r.id === id; });
  if (!rel) return;
...
function _commitRelationLabel(id, value) {
  _ensureRelations();
  var rel = state.relations.find(function(r) { return r.id === id; });
...
function _handleRelationClick(target) {
